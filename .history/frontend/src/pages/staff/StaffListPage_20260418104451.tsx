...
import {
  ...
  Edit2, Trash2, Eye, UserCheck, UserX, Filter
} from 'lucide-react'
...
export function StaffListPage() {
  ...
  const handleStatusChange = async (status: 'active' | 'suspended' | 'on_leave' | 'terminated') => {
    if (selectedRows.length === 0) return
    try {
      await staffApi.updateStatusBulk(selectedRows, status)
      setSelectedRows([])
      fetchStaff()
    } catch (err: unknown) {
      const error = err as { response?: { data?: { message?: string } } }
  ...
  const getSelectedStatus = () => {
    ...
    const selected = data.filter(s => selectedRows.includes(s.id))
    if (selected.every(s => s.employment_status === 'active')) return 'active'
    if (selected.every(s => s.employment_status !== 'active')) return 'inactive'
    return 'mixed'
  }

  const hasAdminSelected = () => {
    const selected = data.filter(s => selectedRows.includes(s.id))
    return selected.some(s => s.user?.roles?.some(r => r.name === 'Main Office'))
  }
    ...
    <Layout>
      <div className={`space-y-4 ${fontClass}`} dir={direction}>
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            {selectedRows.length > 0 && !hasAdminSelected() && (
              <>
                ...
                {getSelectedStatus() !== 'inactive' && getSelectedStatus() !== 'mixed' && (
                  <Button 
                    variant="outline" 
                    size="sm"
