...
export function AssignTeamsModal({
  ...
  const loadTeams = async () => {
    ...
    try {
      ...
      const unassigned = Array.isArray(unassignedRes.data)
        ? unassignedRes.data
        : unassignedRes.data?.data || [];

      const allRes = await teamBasedCodeApi.getAllCodes();
      const allTeamsData = Array.isArray(allRes.data)
        ? allRes.data
        : allRes.data?.data || [];

      const ownAssignedTeams = allTeamsData.filter((t: TeamCode) =>
  ...
  const filterAvailable = allTeams.filter(
    (t) =>
      ...
      footer={
        <div className="flex items-center justify-between">
          <span className="text-sm text-gray-500">
            {assignedIds.length}{" "}
            {locale === "ar" ? "فريق معين" : "team(s) assigned"}
          </span>
          ...
          <div className="space-y-2 max-h-72 overflow-y-auto">
            {filterAssigned.length === 0 ? (
              ...
              filterAssigned.map((team) => (
                <div
                  key={team.id}
                  className="group flex items-center gap-3 p-3 bg-linear-to-r from-red-50 to-red-100/50 border border-red-200 rounded-xl hover:from-red-100 hover:to-red-100 transition-all"
                >
        ...
        <div className="space-y-4">
          ...
          <div className="relative">
            ...
            <input
              type="text"
              placeholder={
                locale === "ar" ? "بحث عن كود الفريق..." : "Search team code..."
              }
              value={search}
          ...
          <div className="mt- flex items-center justify-between px-2">
            <h4 className="font-semibold text-gray-800 flex items-center gap-2">
