...
<body>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="{{ $professional->is_active ? 'status-active' : 'status-inactive' }}">
                    {{ $professional->is_active ? 'Active' : 'Inactive' }}
                </td>
                <td>{{ $professional->created_at?->format('Y-m-d H:i:s') ?: '-' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="footer">
        <p>Total Records: {{ $professionals->count() }}</p>
        <p>PHC Evaluation System - Professionals Export</p>
    </div>
</body>
</html>
