...
<head>
    ...
    <title>Educational Degrees Export</title>
    <style>
...
<body>
    <div class="header">
        <h1>Educational Degrees Report</h1>
    ...
    <table>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td class="{{ $degree->is_active ? 'status-active' : 'status-inactive' }}">
                    {{ $degree->is_active ? 'Active' : 'Inactive' }}
                </td>
                <td>{{ $degree->created_at?->format('Y-m-d H:i:s') ?: '-' }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
    ...
    <div class="footer">
        <p>Total Records: {{ $degrees->count() }}</p>
        <p>PHC Evaluation System - Educational Degrees Export</p>
    </div>
</body>
</html>
