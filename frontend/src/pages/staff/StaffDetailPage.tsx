...
interface StaffMember {
  ...
  nationality?: { id: number; name: string; name_ar: string }
  birth_date?: string
  ...
  shc_category_id?: number
  shc_category?: { id: number; code: string; description?: string; medical_field_id: number; medical_field?: { id: number; name: string; name_ar: string }; specialty_id: number; specialty?: { id: number; name: string; name_ar: string }; rank_id: number; rank?: { id: number; name: string; name_ar: string } }
...
export function StaffDetailPage() {
  ...
  const calculateExperience = (hireDateStr?: string) => {
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">{locale === 'ar' ? 'الجنسية' : 'Nationality'}</p>
                  <p className="font-medium">
                    {staff.nationality?.name || staff.nationality?.name_ar || '-'}
                  </p>
                </div>
              </div>
            </div>
...
<div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <p className="text-sm text-gray-500">{locale === 'ar' ? 'الحقل الطبي' : 'Medical Field'}</p>
                  <p className="font-medium">
                    {staff.shc_category?.medical_field?.name || staff.shc_category?.medical_field?.name_ar || '-'}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">{locale === 'ar' ? 'التخصص' : 'Specialty'}</p>
                  <p className="font-medium">
                    {staff.shc_category?.specialty?.name || staff.shc_category?.specialty?.name_ar || '-'}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-gray-500">{locale === 'ar' ? 'الرتبة' : 'Rank'}</p>
                  <p className="font-medium">
                    {staff.shc_category?.rank?.name || staff.shc_category?.rank?.name_ar || '-'}
                  </p>
                </div>
                <div className="col-span-2">
                  <p className="text-sm text-gray-500">{locale === 'ar' ? 'فئة الهيئة' : 'SHC Category'}</p>
                  {staff.shc_category ? (
                    <span className="inline-flex items-center px-3 py-1.5 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                      {staff.shc_category.code}
                    </span>
                  ) : (
                ...
                <div>
                  ...
                  {staff.hire_date && (() => {
                    const exp = calculateExperience(staff.hire_date)
                    if (!exp) return null
                    return (
                      <p className="text-sm text-gray-500 mt-1">
                        {exp.years} {locale === 'ar' ? (exp.years === 1 ? 'سنة خبرة' : 'سنوات خبرة') : exp.years === 1 ? 'Year exp.' : 'Years exp.'}
                        {exp.months > 0 && `, ${exp.months} ${locale === 'ar' ? 'شهر' : 'Months'}`}
                      </p>
                    )
                  })()}
                </div>
