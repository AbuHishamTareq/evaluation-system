...
import { Search, Plus, Edit2, Trash2, MapPin, ChevronLeft, ChevronRight, ChevronFirst, ChevronLast, FileSpreadsheet, GraduationCap } from 'lucide-react'
...
export function SpecialtyListPage() {
  ...
  const handleDelete = async (id: number) => {
    const specialty = data.find(n => n.id === id)
    const name = locale === 'ar' ? specialty?.name_ar || specialty?.name : specialty?.name
    ...
    try {
      ...
      Swal.fire({
        title: 'Deleted',
        text: locale === 'ar' ? 'تم حذف التخصص بنجاح' : 'Specialty has been deleted',
        icon: 'success',
      })
    } catch {
      Swal.fire({
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      await specialtyApi.toggleStatus(id)
      fetchData()
      Swal.fire({
        title: currentStatus ? 'Deactivated' : 'Activated',
        text: currentStatus
          ? (locale === 'ar' ? 'تم إلغاء تفعيل التخصص' : 'Specialty has been deactivated')
          : (locale === 'ar' ? 'تم تفعيل التخصص بنجاح' : 'Specialty has been activated'),
        icon: 'success',
      })
    } catch (err: unknown) {
  ...
  const columns = [
    ...
    columnHelper.accessor(row => row.medical_field?.name || '-', {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {locale === 'ar' ? 'إدارة التخصصات' : 'Specialty Management'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {totalCount} {locale === 'ar' ? 'تخصص' : 'specialties'}
            </p>
          </div>
                  ...
                  <tr>
                    <td colSpan={columns.length} className="px-4 py-8 text-center">
                      ...
                      <p className="text-gray-500">
                        {locale === 'ar' ? 'لا توجد تخصصات' : 'No specialties found'}
                      </p>
                    </td>
                  </tr>
                ) : (
