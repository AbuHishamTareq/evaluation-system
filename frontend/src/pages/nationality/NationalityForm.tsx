...
export function NationalityForm() {
  ...
  const fetchData = async () => {
    ...
    setIsLoading(true)
    try {
      const res = await nationalityApi.getById(Number(id))
      const nationality = res.data.data
      setFormData({
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit && id) {
        await nationalityApi.update(Number(id), {
          name: formData.name,
      ...
      <div className="max-w-2xl mx-auto">
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-6">
            {isEdit
              ? locale === 'ar' ? 'تعديل الجنسية' : 'Edit Nationality'
          ...
          <div className="flex justify-end gap-3 mt-6">
            <Button variant="secondary" onClick={() => navigate('/nationalities')}>
