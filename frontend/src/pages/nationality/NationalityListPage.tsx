...
import { nationalityApi } from '@/lib/api'
...
const columnHelper = createColumnHelper<Nationality>()

export function NationalityListPage() {
  ...
  const canCreate = hasPermission('nationalities.create')
  const canEdit = hasPermission('nationalities.edit')
  const canDelete = hasPermission('nationalities.delete')
  const canToggle = hasPermission('nationalities.toggle')
  const canImportExport = hasPermission('nationalities.import_export')
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await nationalityApi.getAll(params)
  ...
  const handleDelete = async (id: number) => {
    const nationality = data.find(n => n.id === id)
    const name = locale === 'ar' ? nationality?.name_ar || nationality?.name : nationality?.name
    ...
    try {
      ...
      Swal.fire({
        title: 'Deleted',
        text: locale === 'ar' ? 'تم حذف الجنسية بنجاح' : 'Nationality has been deleted',
        icon: 'success',
      })
    } catch {
      Swal.fire({
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      await nationalityApi.toggleStatus(id)
      fetchData()
      Swal.fire({
        title: currentStatus ? 'Deactivated' : 'Activated',
        text: currentStatus
          ? (locale === 'ar' ? 'تم إلغاء تفعيل الجنسية' : 'Nationality has been deactivated')
          : (locale === 'ar' ? 'تم تفعيل الجنسية بنجاح' : 'Nationality has been activated'),
        icon: 'success',
      })
    } catch (err: unknown) {
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          {canEdit && (
            <button
              onClick={() => navigate(`/nationalities/${row.original.id}`)}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {locale === 'ar' ? 'إدارة الجنسيات' : 'Nationality Management'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {totalCount} {locale === 'ar' ? 'جنسية' : 'nationalities'}
            </p>
          </div>
          <div className="flex items-center gap-2">
            ...
            {canCreate && (
            <Link to="/nationalities/new">
              <Button size="sm">
                <Plus className="w-4 h-4 me-2" />
                {locale === 'ar' ? 'إضافة جنسية' : 'Add Nationality'}
              </Button>
            </Link>
          )}
          </div>
                  ...
                  <tr>
                    <td colSpan={columns.length} className="px-4 py-8 text-center">
                      ...
                      <p className="text-gray-500">
                        {locale === 'ar' ? 'لا توجد جنسيات' : 'No nationalities found'}
                      </p>
                    </td>
                  </tr>
                ) : (
