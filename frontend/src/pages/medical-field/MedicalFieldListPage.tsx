...
const columnHelper = createColumnHelper<MedicalField>()

export function MedicalFieldListPage() {
  ...
  const canCreate = hasPermission('medical_fields.create')
  const canEdit = hasPermission('medical_fields.edit')
  const canDelete = hasPermission('medical_fields.delete')
  const canToggle = hasPermission('medical_fields.toggle')
  const canImportExport = hasPermission('medical_fields.import_export')
  const [data, setData] = useState<MedicalField[]>([])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await medicalFieldApi.getAll(params)
  ...
  const handleDelete = async (id: number) => {
    const medicalField = data.find(n => n.id === id)
    const name = locale === 'ar' ? medicalField?.name_ar || medicalField?.name : medicalField?.name
    ...
    try {
      await medicalFieldApi.delete(id)
      fetchData()
      Swal.fire({
        title: 'Deleted',
        text: locale === 'ar' ? 'تم حذف الحقل الطبي بنجاح' : 'Medical Field has been deleted',
        icon: 'success',
      })
    } catch {
      Swal.fire({
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      await medicalFieldApi.toggleStatus(id)
      fetchData()
      Swal.fire({
        title: currentStatus ? 'Deactivated' : 'Activated',
        text: currentStatus
          ? (locale === 'ar' ? 'تم إلغاء تفعيل الحقل الطبي' : 'Medical Field has been deactivated')
          : (locale === 'ar' ? 'تم تفعيل الحقل الطبي بنجاح' : 'Medical Field has been activated'),
        icon: 'success',
      })
    } catch (err: unknown) {
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          {canEdit && (
            <button
              onClick={() => navigate(`/medical-fields/${row.original.id}`)}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {locale === 'ar' ? 'إدارة الحقول الطبية' : 'Medical Field Management'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {totalCount} {locale === 'ar' ? 'حقل طبي' : 'medical fields'}
            </p>
          </div>
                  ...
                  <tr>
                    <td colSpan={columns.length} className="px-4 py-8 text-center">
                      ...
                      <p className="text-gray-500">
                        {locale === 'ar' ? 'لا توجد حقول طبية' : 'No medical fields found'}
                      </p>
                    </td>
                  </tr>
                ) : (
