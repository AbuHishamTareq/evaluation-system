...
import { Search, Plus, Edit2, Trash2, Building2, Filter, ChevronLeft, ChevronRight, ChevronFirst, ChevronLast, FileSpreadsheet } from 'lucide-react'
...
export function DepartmentListPage() {
  ...
  const canToggle = hasPermission('departments.toggle')
  const canImportExport = hasPermission('departments.import_export')
  ...
  const [phcCenterFilter, setPhcCenterFilter] = useState('')
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (phcCenterFilter) params.phc_center_id = phcCenterFilter
  ...
  useEffect(() => {
    fetchData()
  }, [search, phcCenterFilter, pagination.pageIndex, pagination.pageSize])
  ...
  const handleDelete = async (id: number) => {
    ...
    const name = locale === 'ar' ? dept?.name_ar || dept?.name : dept?.name
    ...
    try {
      ...
      Swal.fire({
        title: 'Deleted',
        text: locale === 'ar' ? 'تم حذف القسم بنجاح' : 'Department has been deleted',
        icon: 'success',
      })
    } catch (err) {
      Swal.fire({
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      ...
      Swal.fire({
        ...
        text: currentStatus
          ? (locale === 'ar' ? 'تم إلغاء تفعيل القسم' : 'Department has been deactivated')
          : (locale === 'ar' ? 'تم تفعيل القسم بنجاح' : 'Department has been activated'),
        icon: 'success',
      })
    } catch (err: unknown) {
  ...
  const columns = [
    ...
    columnHelper.accessor('name', {
      header: locale === 'ar' ? 'اسم القسم' : 'Department Name',
    ...
    columnHelper.accessor(
      (row) => row.phc_center?.name || '-',
      {
        id: 'phc_center',
        header: locale === 'ar' ? 'مركز الصحة' : 'PHC Center',
      }
    ),
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              {totalCount} {locale === 'ar' ? 'قسم' : 'departments'}
            </p>
          </div>
            ...
            <div className="p-4 border-b border-gray-200 bg-gray-50">
              <div className="space-y-3">
                <div>
                  ...
                  <select
                    value={phcCenterFilter}
                    onChange={(e) => setPhcCenterFilter(e.target.value)}
