...
export function ZoneListPage() {
  ...
  const canToggle = hasPermission('zones.toggle')
  const canImportExport = hasPermission('zones.import_export')
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await zoneApi.getAll(params)
  ...
  const handleDelete = async (id: number) => {
    ...
    const name = locale === 'ar' ? zone?.name_ar || zone?.name : zone?.name
    ...
    try {
      ...
      Swal.fire({
        title: 'Deleted',
        text: locale === 'ar' ? 'تم حذف المنطقة بنجاح' : 'Zone has been deleted',
        icon: 'success',
      })
    } catch (err) {
      Swal.fire({
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      await zoneApi.toggleStatus(id)
      fetchData()
      Swal.fire({
        title: currentStatus ? 'Deactivated' : 'Activated',
        text: currentStatus
          ? (locale === 'ar' ? 'تم إلغاء تفعيل المنطقة' : 'Zone has been deactivated')
          : (locale === 'ar' ? 'تم تفعيل المنطقة بنجاح' : 'Zone has been activated'),
        icon: 'success',
      })
    } catch (err: unknown) {
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {locale === 'ar' ? 'إدارة المناطق' : 'Zone Management'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {totalCount} {locale === 'ar' ? 'منطقة' : 'zones'}
            </p>
          </div>
                  ...
                  <tr>
                    <td colSpan={columns.length} className="px-4 py-8 text-center">
                      ...
                      <p className="text-gray-500">
                        {locale === 'ar' ? 'لا توجد مناطق' : 'No zones found'}
                      </p>
                    </td>
                  </tr>
                ) : (
