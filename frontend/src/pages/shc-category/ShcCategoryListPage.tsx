...
import { shcCategoryApi } from '@/lib/api'
...
interface ShcCategory {
  ...
  specialty_id: number
  specialty?: { id: number; name: string; name_ar: string }
  rank_id: number
  rank?: { id: number; name: string; name_ar: string }
  is_active: boolean
}

const columnHelper = createColumnHelper<ShcCategory>()

export function ShcCategoryListPage() {
  ...
  const canCreate = hasPermission('shc_categories.create')
  const canEdit = hasPermission('shc_categories.edit')
  const canDelete = hasPermission('shc_categories.delete')
  const canToggle = hasPermission('shc_categories.toggle')
  const canImportExport = hasPermission('shc_categories.import_export')
  const [data, setData] = useState<ShcCategory[]>([])
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await shcCategoryApi.getAll(params)
      ...
      console.error('Failed to load shc categories:', err)
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const handleDelete = async (id: number) => {
    const category = data.find(n => n.id === id)
    const name = locale === 'ar' ? category?.description_ar || category?.description : category?.description
    ...
    try {
      await shcCategoryApi.delete(id)
      fetchData()
      Swal.fire({
        title: 'Deleted',
        text: locale === 'ar' ? 'تم حذف الفئة بنجاح' : 'Category has been deleted',
        icon: 'success',
      })
    } catch {
      Swal.fire({
  ...
  const handleToggleStatus = async (id: number, currentStatus: boolean) => {
    try {
      await shcCategoryApi.toggleStatus(id)
      fetchData()
      Swal.fire({
        title: currentStatus ? 'Deactivated' : 'Activated',
        text: currentStatus
          ? (locale === 'ar' ? 'تم إلغاء تفعيل الفئة' : 'Category has been deactivated')
          : (locale === 'ar' ? 'تم تفعيل الفئة بنجاح' : 'Category has been activated'),
        icon: 'success',
      })
    } catch (err: unknown) {
  ...
  const getFieldName = (category: ShcCategory) => {
    return locale === 'ar' ? category.medical_field?.name_ar || category.medical_field?.name : category.medical_field?.name
  }

  const getSpecialtyName = (category: ShcCategory) => {
    return locale === 'ar' ? category.specialty?.name_ar || category.specialty?.name : category.specialty?.name
  }

  const getRankName = (category: ShcCategory) => {
    return locale === 'ar' ? category.rank?.name_ar || category.rank?.name : category.rank?.name
  }
  ...
  const columns = [
    ...
    columnHelper.accessor((row) => getSpecialtyName(row), {
      id: 'specialty',
      header: locale === 'ar' ? 'التخصص' : 'Specialty',
    }),
    columnHelper.accessor((row) => getRankName(row), {
      id: 'rank',
      header: locale === 'ar' ? 'الرتبة' : 'Rank',
    }),
    ...
    columnHelper.display({
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-1">
          {canEdit && (
            <button
              onClick={() => navigate(`/shc-categories/${row.original.id}`)}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">
              {locale === 'ar' ? 'إدارة فئات الهيئة' : 'SHC Category Management'}
            </h1>
            <p className="text-sm text-gray-500 mt-1">
              {totalCount} {locale === 'ar' ? 'فئة' : 'categories'}
            </p>
          </div>
