...
export const TeamCodeCard: React.FC<TeamCodeCardProps> = ({
  teamCode,
  onClick,
  onEdit,
  onDelete,
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-start gap-4 flex-1 min-w-0">
          <div className="w-12 h-12 rounded-lg flex items-center justify-center font-semibold text-sm bg-gradient-to-br from-amber-100 to-yellow-100 text-amber-700">
            {teamCode.code.replace('TBC-', '')}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 flex-wrap">
              <h4 className="font-semibold text-gray-900 truncate">{teamCode.code}</h4>
              {onToggleActive && (
                <button
                  type="button"
                  onClick={(e) => {
                    e.stopPropagation();
                    onToggleActive(teamCode, !teamCode.is_active);
                  }}
            ...
            {teamCode.description && (
              <p className="text-sm text-gray-500 mt-1 line-clamp-1">{teamCode.description}</p>
            )}
