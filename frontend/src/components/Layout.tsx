...
function NavGroupComponent({ group, isSidebarCollapsed, setIsMobileMenuOpen }: { group: NavGroup; isSidebarCollapsed: boolean; setIsMobileMenuOpen: (v: boolean) => void }) {
  ...
  return (
    <div className="relative group">
      ...
      {isSidebarCollapsed && (
        <div className="absolute start-full -top-2 py-2 px-0 space-y-1 bg-surface border border-primary/20 rounded-lg shadow-lg p-2 min-w-[180px] z-50 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200">
...
const navGroups: (NavGroup | NavItem)[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { key: 'team-based-codes', icon: Code, path: '/team-based-codes' },
    ],
  },
          ...
          <div className="flex items-center justify-between h-16 px-4">
            <div className="flex items-center gap-4">
              <button
                ...
                className="p-2.5 text-muted hover:text-primary rounded-xl transition-all duration-300 hover:bg-gradient-to-r hover:from-primary/10 hover:to-secondary/10 md:hidden"
              >
